...
public class ChunkedInputStream extends InputStream {
    ...
    /* renamed from: in */
    private SessionInputBuffer f377in;
    private int pos;
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        if (this.closed) {
            ...
            int bytesRead = this.f377in.read(b, off, Math.min(len, this.chunkSize - this.pos));
            if (bytesRead != -1) {
                this.pos += bytesRead;
                return bytesRead;
            }
    ...
    private int getChunkSize() throws IOException {
        if (!this.bof) {
            int cr = this.f377in.read();
            int lf = this.f377in.read();
            if (!(cr == 13 && lf == 10)) {
                throw new MalformedChunkCodingException("CRLF expected at end of chunk");
            }
        }
        this.buffer.clear();
        if (this.f377in.readLine(this.buffer) == -1) {
            return 0;
        }
    ...
    private void parseTrailerHeaders() throws IOException {
        try {
            this.footers = AbstractMessageParser.parseHeaders(this.f377in, -1, -1, (LineParser) null);
