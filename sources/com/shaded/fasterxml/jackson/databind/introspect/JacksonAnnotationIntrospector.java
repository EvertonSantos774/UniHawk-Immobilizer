...
public class JacksonAnnotationIntrospector extends AnnotationIntrospector implements Serializable {
    ...
    public Class<?> findSerializationType(Annotated annotated) {
        Class<?> as;
        JsonSerialize jsonSerialize = (JsonSerialize) annotated.getAnnotation(JsonSerialize.class);
        if (jsonSerialize == null || (as = jsonSerialize.mo21723as()) == NoClass.class) {
            return null;
        }
        return as;
    }
    ...
    public Class<?> findDeserializationType(Annotated annotated, JavaType javaType) {
        Class<?> as;
        JsonDeserialize jsonDeserialize = (JsonDeserialize) annotated.getAnnotation(JsonDeserialize.class);
        if (jsonDeserialize == null || (as = jsonDeserialize.mo21711as()) == NoClass.class) {
            return null;
        }
        return as;
    }
    ...
    public TypeResolverBuilder<?> _findTypeResolver(MapperConfig<?> mapperConfig, Annotated annotated, JavaType javaType) {
        ...
        if (jsonTypeResolver != null) {
            ...
            if (jsonTypeInfo.use() == JsonTypeInfo.C1448Id.NONE) {
        ...
        JsonTypeInfo.C1447As include = jsonTypeInfo.include();
        if (include == JsonTypeInfo.C1447As.EXTERNAL_PROPERTY && (annotated instanceof AnnotatedClass)) {
            include = JsonTypeInfo.C1447As.PROPERTY;
        }
