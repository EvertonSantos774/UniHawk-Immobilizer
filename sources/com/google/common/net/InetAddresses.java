...
public final class InetAddresses {
    ...
    private static final Splitter IPV4_SPLITTER = Splitter.m62on('.').limit(4);
    private static final int IPV6_PART_COUNT = 8;
    private static final Splitter IPV6_SPLITTER = Splitter.m62on(':').limit(10);
    ...
    private static byte[] textToNumericFormatV4(String ipString) {
        ...
        int i2 = 0;
        try {
            Iterator<String> it = IPV4_SPLITTER.split(ipString).iterator();
            while (true) {
                try {
                    i = i2;
                    if (!it.hasNext()) {
    ...
    public static boolean isTeredoAddress(Inet6Address ip) {
        byte[] bytes = ip.getAddress();
        if (bytes[0] == 32 && bytes[1] == 1 && bytes[2] == 0 && bytes[3] == 0) {
            return true;
        }
        return false;
    }
    ...
    public static InetAddress increment(InetAddress address) {
        ...
        Preconditions.checkArgument(z, "Incrementing %s would wrap.", (Object) address);
        addr[i] = (byte) (addr[i] + 1);
        return bytesToInetAddress(addr);
    }
