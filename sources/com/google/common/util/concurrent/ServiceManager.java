...
public final class ServiceManager {
    ...
    public ServiceManager(Iterable<? extends Service> services2) {
        ...
        if (copy.isEmpty()) {
            ...
            copy = ImmutableList.m81of(new NoOpService());
        }
    ...
    public String toString() {
        return MoreObjects.toStringHelper((Class<?>) ServiceManager.class).add(ComponentDescriptorConstants.SERVICES_TARGET, (Object) Collections2.filter(this.services, Predicates.not(Predicates.instanceOf(NoOpService.class)))).toString();
    }
        ...
        public void awaitHealthy(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.awaitHealthGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to become healthy. The following services have not started: " + Multimaps.filterKeys(this.servicesByState, Predicates.m55in(ImmutableSet.m124of(Service.State.NEW, Service.State.STARTING))));
                }
                checkHealthy();
            } finally {
                this.monitor.leave();
        ...
        public void awaitStopped(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.stoppedGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to stop. The following services have not stopped: " + Multimaps.filterKeys(this.servicesByState, Predicates.not(Predicates.m55in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
                }
            } finally {
                this.monitor.leave();
            }
