    ...
    public static HashCode combineOrdered(Iterable<HashCode> hashCodes) {
        ...
        for (HashCode hashCode : hashCodes) {
            ...
            for (int i = 0; i < nextBytes.length; i++) {
                resultBytes[i] = (byte) ((resultBytes[i] * Ev3Constants.Opcode.AND16) ^ nextBytes[i]);
            }
        }
    ...
    private static final class ConcatenatedHashFunction extends AbstractCompositeHashFunction {
        private ConcatenatedHashFunction(HashFunction... functions) {
            ...
            for (HashFunction function : functions) {
                if (function.bits() % 8 == 0) {
                    z = true;
                } else {
                    z = false;
                }
                Preconditions.checkArgument(z, "the number of bits (%s) in hashFunction (%s) must be divisible by 8", function.bits(), (Object) function);
            }
        }
