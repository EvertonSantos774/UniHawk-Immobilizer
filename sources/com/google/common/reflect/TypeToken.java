...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable {
    ...
    /* renamed from: of */
    public static <T> TypeToken<T> m220of(Class<T> type) {
        return new SimpleTypeToken(type);
    }

    /* renamed from: of */
    public static TypeToken<?> m221of(Type type) {
        return new SimpleTypeToken(type);
    }
    ...
    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, TypeToken<X> typeArg) {
        return new SimpleTypeToken(new TypeResolver().where(ImmutableMap.m100of(new TypeResolver.TypeVariableKey(typeParam.typeVariable), typeArg.runtimeType)).resolveType(this.runtimeType));
    }

    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, Class<X> typeArg) {
        return where(typeParam, m220of(typeArg));
    }

    public final TypeToken<?> resolveType(Type type) {
        Preconditions.checkNotNull(type);
        return m221of(getInvariantTypeResolver().resolveType(type));
    }

    private TypeToken<?> resolveSupertype(Type type) {
        TypeToken<?> supertype = m221of(getCovariantTypeResolver().resolveType(type));
    ...
    public final TypeToken<? extends T> getSubtype(Class<?> subclass) {
        ...
        TypeToken<?> of = m221of(resolveTypeArgsForSubclass(subclass));
    ...
    public final boolean isSupertypeOf(Type type) {
        return m221of(type).isSubtypeOf(getType());
    }
    ...
    public final boolean isSubtypeOf(Type supertype) {
        ...
        if (this.runtimeType instanceof GenericArrayType) {
            return m221of(supertype).isSupertypeOfArray((GenericArrayType) this.runtimeType);
        }
    ...
    public final TypeToken<T> wrap() {
        if (isPrimitive()) {
            return m220of(Primitives.wrap((Class) this.runtimeType));
        }
        return this;
    }
    ...
    public final TypeToken<T> unwrap() {
        if (isWrapper()) {
            return m220of(Primitives.unwrap((Class) this.runtimeType));
        }
        return this;
    }
    ...
    public final Invokable<T, T> constructor(Constructor<?> constructor) {
        ...
        return new Invokable.ConstructorInvokable<T>(constructor) {
            ...
            public String toString() {
                return getOwnerType() + "(" + Joiner.m50on(", ").join((Object[]) getGenericParameterTypes()) + ")";
            }
        };
    }
    ...
    public Object writeReplace() {
        return m221of(new TypeResolver().resolveType(this.runtimeType));
    }
    ...
    private boolean isSubtypeOfParameterizedType(ParameterizedType supertype) {
        boolean z;
        Class<? super Object> rawType = m221of((Type) supertype).getRawType();
        ...
        for (int i = 0; i < typeVars.length; i++) {
            if (!m221of(getCovariantTypeResolver().resolveType(typeVars[i])).m219is(supertypeArgs[i], typeVars[i])) {
                return false;
            }
        }
    ...
    private boolean isSubtypeOfArrayType(GenericArrayType supertype) {
        if (this.runtimeType instanceof Class) {
            ...
            return m220of(fromClass.getComponentType()).isSubtypeOf(supertype.getGenericComponentType());
        } else if (this.runtimeType instanceof GenericArrayType) {
            return m221of(((GenericArrayType) this.runtimeType).getGenericComponentType()).isSubtypeOf(supertype.getGenericComponentType());
        } else {
            return false;
        }
    }
    ...
    private boolean isSupertypeOfArray(GenericArrayType subtype) {
        if (this.runtimeType instanceof Class) {
            ...
            return m221of(subtype.getGenericComponentType()).isSubtypeOf((Type) thisClass.getComponentType());
        } else if (this.runtimeType instanceof GenericArrayType) {
            return m221of(subtype.getGenericComponentType()).isSubtypeOf(((GenericArrayType) this.runtimeType).getGenericComponentType());
        } else {
            return false;
        }
    }
    ...
    /* renamed from: is */
    private boolean m219is(Type formalType, TypeVariable<?> declaration) {
    ...
    private static class Bounds {
        ...
        public boolean isSubtypeOf(Type supertype) {
            for (Type bound : this.bounds) {
                if (TypeToken.m221of(bound).isSubtypeOf(supertype) == this.target) {
                    return this.target;
                }
            }
            if (!this.target) {
        ...
        public boolean isSupertypeOf(Type subtype) {
            TypeToken<?> type = TypeToken.m221of(subtype);
            for (Type bound : this.bounds) {
    ...
    public ImmutableSet<Class<? super T>> getRawTypes() {
        ...
        new TypeVisitor() {
            ...
            public void visitGenericArrayType(GenericArrayType t) {
                builder.add((Object) Types.getArrayClass(TypeToken.m221of(t.getGenericComponentType()).getRawType()));
            }
        }.visit(this.runtimeType);
        return builder.build();
    }
    ...
    private boolean isOwnedBySubtypeOf(Type supertype) {
        ...
        while (it.hasNext()) {
            ...
            if (ownerType != null && m221of(ownerType).isSubtypeOf(supertype)) {
                return true;
            }
        }
        return false;
    ...
    static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
        if (cls.isArray()) {
            return m221of(Types.newArrayType(toGenericType(cls.getComponentType()).runtimeType));
        }
        ...
        if (typeParams.length > 0 || (ownerType != null && ownerType != cls.getEnclosingClass())) {
            return m221of((Type) Types.newParameterizedTypeWithOwner(ownerType, cls, typeParams));
        }
        return m220of(cls);
    }
    ...
    private TypeToken<? super T> getSupertypeFromUpperBounds(Class<? super T> supertype, Type[] upperBounds) {
        for (Type upperBound : upperBounds) {
            TypeToken<?> of = m221of(upperBound);
    ...
    private TypeToken<? extends T> getSubtypeFromLowerBounds(Class<?> subclass, Type[] lowerBounds) {
        if (0 < lowerBounds.length) {
            return m221of(lowerBounds[0]).getSubtype(subclass);
        }
        throw new IllegalArgumentException(subclass + " isn't a subclass of " + this);
    }

    private TypeToken<? super T> getArraySupertype(Class<? super T> supertype) {
        return m221of(newArrayClassOrGenericArrayType(((TypeToken) Preconditions.checkNotNull(getComponentType(), "%s isn't a super type of %s", (Object) supertype, (Object) this)).getSupertype(supertype.getComponentType()).runtimeType));
    }

    private TypeToken<? extends T> getArraySubtype(Class<?> subclass) {
        return m221of(newArrayClassOrGenericArrayType(getComponentType().getSubtype(subclass.getComponentType()).runtimeType));
    }
    ...
    private static abstract class TypeCollector<K> {
        ...
        public final ImmutableList<K> collectTypes(K type) {
            return collectTypes(ImmutableList.m81of(type));
        }
