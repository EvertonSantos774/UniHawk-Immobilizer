...
public final class CsvUtil {
    ...
    private static class CsvParser implements Iterator<List<String>> {
        ...
        public void skip(long charPosition) throws IOException {
            int n;
            while (charPosition > 0 && (n = this.f33in.read(this.buf, 0, Math.min((int) charPosition, this.buf.length))) >= 0) {
        ...
        private void fill() {
            ...
            while (this.opened && toFill > 0) {
                try {
                    int n = this.f33in.read(this.buf, this.limit, toFill);
                    if (n == -1) {
                        this.opened = false;
                    } else {
                        this.limit += n;
